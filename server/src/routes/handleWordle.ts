...
type TGameSession = {
  ...
  guessCount: number;
  timestamp: number;
  feedback: { letter: string; status: string }[][];
};
...
wordleRouter
  ...
  .post("/start-game", async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const filteredWords = wordlist.filter(
        (word) => word.length === requestedWordLength,
      );

      if (filteredWords.length === 0) {
        res
          .status(400)
          .json({ message: "No words available for the requested length" });
        return;
      }
  ...
  .post("/guess/:id", async (req: Request, res: Response) => {
    ...
    const result = await validateGuess(gameSession.word, req.body.guess);
    gameSession.guessCount++;
    gameSession.feedback.push(result);
    ...
    if (gameWon) {
      Object.assign(responsBody, {
        correctWord: gameSession.word,
        win: true,
        time: getElapsedTime(gameSession.timestamp),
      });
    }

    if (!gameWon && gameSession.guessCount >= gameSession.word.length) {
      Object.assign(responsBody, {
        correctWord: gameSession.word,
        win: false,
        time: getElapsedTime(gameSession.timestamp),
    ...
    console.log("Response: ", responsBody);
    res.status(200).json(responsBody);
  });

const validateGuess = async (word: string, guess: string) => {
