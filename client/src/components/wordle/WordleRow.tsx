...
const WordleRow: React.FC<WordleRowProps> = ({ word, feedback, wordLength }) => {
  return (
    <Flex justify={'center'}>
      {[...word.padEnd(wordLength)].map((letter, i) => {
        const status = feedback ? feedback[i]?.status : '';
        return (
          <LetterFeedbackBox letter={letter} status={StatusColors[status as keyof typeof StatusColors] || StatusColors.default} />
        );
      })}
    </Flex>
  );
